...
func TestNewURLBuilder_PositiveCases(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			&url.URL{Scheme: "https", Host: "example.com:8080", Path: ""},
		},
		{
...
func TestURLBuilder_Build_Positive_Cases(t *testing.T) {
	tests := []struct {
		name     string
		origin   string
		parts    []string
...
func TestURLBuilder_Build_NegativeCases(t *testing.T) {
	tests := []struct {
		name   string
		origin string
		parts  []string
		...
		{"no parts", "https://example.com", nil, ErrEmptyPath},
		{"no parts", "https://example.com", []string{}, ErrEmptyPath},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := ub.Build(tt.parts...)
			if err == nil {
				t.Errorf("failed to build %s. expected error %v", tt.origin, err)
				return
			}

			if got != nil {
				t.Errorf("failed to build %s. expected empty url.URL", tt.origin)
				return
			}
		})
	}
